...
export function CourseListCard({ course }: CourseListCardProps) {
  return (
    ...
    >
      <div
        className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-xl ${course.swatch} shadow-inner`}
        aria-hidden
      />
      <div className="min-w-0 flex-1">
        <p className="font-bold leading-snug text-ink">{course.title}</p>
        <p className="mt-1 text-sm text-ink-secondary">{course.provider}</p>
